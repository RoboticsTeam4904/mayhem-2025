...
public class SwerveDrive {
     ...
     */
    private final Alert tunerXRecommendation = new Alert(
        "Swerve Drive",
    ...
    private final DoublePublisher rawIMUPublisher = NetworkTableInstance.getDefault()
        .getDoubleTopic("swerve/Raw IMU Yaw")
        .publish();
    /**
    ...
    private final DoublePublisher adjustedIMUPublisher = NetworkTableInstance.getDefault()
        .getDoubleTopic("swerve/Adjusted IMU Yaw")
        .publish();
    /**
     * Field object.
     */
        ...
        if (SwerveDriveTelemetry.isSimulation) {
            ...
            for (int i = 0; i < swerveModules.length; i++) {
                this.swerveModules[i].configureModuleSimulation(
                        mapleSimDrive.getModules()[i],
                        config.physicalCharacteristics
                    );
            }
    ...
    public void driveFieldOrientedAndRobotOriented(
        ...
        drive(
            ChassisSpeeds.fromFieldRelativeSpeeds(fieldOrientedVelocity, getOdometryHeading()).plus(
    ...
    public void drive(
        ...
        robotRelativeVelocity = movementOptimizations(
        ...
        if (headingCorrection) {
            if (
                Math.abs(robotRelativeVelocity.omegaRadiansPerSecond) <
                ...
                robotRelativeVelocity.omegaRadiansPerSecond = swerveController.headingCalculate(
                    getOdometryHeading().getRadians(),
                    lastHeadingRadians
                );
            } else {
    ...
    private void setRawModuleStates(
        ...
        if (
            attainableMaxTranslationalSpeedMetersPerSecond != 0 ||
    ...
    public void drive(
        ...
        for (SwerveModule module : swerveModules) {
            ...
            double feedforwardVoltage = driveMotorModel.getVoltage(
                ...
                (feedforwardForces[module.moduleNumber].in(Newtons) * wheelRadiusMeters) /
                driveGearRatio,
    ...
    public void setChassisSpeeds(ChassisSpeeds robotRelativeSpeeds) {
        SwerveDriveTelemetry.startCtrlCycle();
        robotRelativeSpeeds = movementOptimizations(
            robotRelativeSpeeds,
    ...
    public Optional<Pose2d> getSimulationDriveTrainPose() {
        odometryLock.lock();
        Optional<Pose2d> simulationPose = getMapleSimDrive()
            .map(AbstractDriveTrainSimulation::getSimulatedDriveTrainPose);
        odometryLock.unlock();
        ...
        try {
            ...
            if (SwerveDriveTelemetry.verbosity.ordinal() >= TelemetryVerbosity.POSE.ordinal()) {
                if (SwerveDriveTelemetry.isSimulation) {
                    ...
                    field
                        .getObject("OdometryPose")
                        .setPose(swerveDrivePoseEstimator.getEstimatedPosition());
                    field
                        .getObject("XModules")
