...
public class SparkFlexSwerve extends SwerveMotor {
    ...
    public SparkFlexSwerve(CANSparkFlex motor, boolean isDriveMotor) {
        ...
        absoluteEncoderOffsetWarning = new Alert(
            "Motors",
    ...
    public void configureIntegratedEncoder(double positionConversionFactor) {
        if (absoluteEncoder == null) {
            ...
            configureSparkFlex(() -> encoder.setVelocityConversionFactor(positionConversionFactor));
            ...
            configureSparkFlex(() -> {
                if (absoluteEncoder.getAbsoluteEncoder() instanceof AbsoluteEncoder) {
                    return (
                        (AbsoluteEncoder) absoluteEncoder.getAbsoluteEncoder()
                    ).setPositionConversionFactor(positionConversionFactor);
                } else {
                    return (
                        (SparkAnalogSensor) absoluteEncoder.getAbsoluteEncoder()
                    ).setPositionConversionFactor(positionConversionFactor);
                }
            });
            configureSparkFlex(() -> {
