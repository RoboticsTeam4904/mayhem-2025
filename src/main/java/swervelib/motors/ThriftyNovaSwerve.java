...
public class ThriftyNovaSwerve extends SwerveMotor {
    ...
    public ThriftyNovaSwerve(ThriftyNova motor, boolean isDriveMotor, DCMotor motorType) {
        ...
        if (isDriveMotor) {
            ...
            velocityConversion = new Conversion(
                VelocityUnit.ROTATIONS_PER_SEC,
    ...
    public void factoryDefaults() {
        ...
        if (!factoryDefaultOccurred) {
            ...
            DriverStation.reportWarning(
                "Factory defaults not implemented for ThriftyNovaSwerve",
                true
            );
    ...
    private void checkErrors(String message) {
        ...
        if (errors.size() > 0) {
            for (ThriftyNova.Error error : errors) {
                DataLogManager.log(
                    this.getClass().getSimpleName() + ": " + message + error.toString()
                );
                System.out.println(
                    this.getClass().getSimpleName() + ": " + message + error.toString()
                );
