...
public class SwerveModule {
    ...
    public void setDesiredState(SwerveModuleState desiredState, boolean isOpenLoop, boolean force) {
        ...
        if (!force && antiJitterEnabled) {
            ...
            SwerveMath.antiJitter(
                desiredState,
                lastState,
                Math.min(maxDriveVelocity.in(MetersPerSecond), 4)
            );
        }
        ...
        setDesiredState(
            ...
            driveMotorFeedforward.calculateWithVelocities(
                curVelocity.in(MetersPerSecond),
    ...
    public void setDesiredState(
        ...
        if (absoluteEncoder != null && synchronizeEncoderQueued && synchronizeEncoderEnabled) {
            ...
            if (
                Math.abs(angleMotor.getPosition() - absoluteEncoderPosition) >=
    ...
    public void pushOffsetsToEncoders() {
        if (absoluteEncoder != null && angleOffset == configuration.angleOffset) {
            ...
            if (
                angleMotor instanceof SparkMaxSwerve ||
    ...
    public LinearVelocity getMaxVelocity() {
        if (maxDriveVelocity == null) {
            maxDriveVelocity = MetersPerSecond.of(
                (RadiansPerSecond.of(driveMotor.getSimMotor().freeSpeedRadPerSec).in(
                        RotationsPerSecond
                    ) /
    ...
    public AngularVelocity getMaxAngularVelocity() {
        if (maxAngularVelocity == null) {
            maxAngularVelocity = RotationsPerSecond.of(
                RadiansPerSecond.of(angleMotor.getSimMotor().freeSpeedRadPerSec).in(
                    RotationsPerSecond
                ) *
