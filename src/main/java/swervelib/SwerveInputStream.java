...
public class SwerveInputStream implements Supplier<ChassisSpeeds> {
    ...
    public SwerveInputStream copy() {
        SwerveInputStream newStream = new SwerveInputStream(
            swerveDrive,
            controllerTranslationX,
            controllerTranslationY
        );
    ...
    public SwerveInputStream scaleTranslation(double scaleTranslation) {
        translationAxisScale = scaleTranslation == 0
            ? Optional.empty()
            : Optional.of(scaleTranslation);
        return this;
    }
    ...
    public ChassisSpeeds get() {
        ...
        switch (newMode) {
            ...
            case AIM -> {
                ...
                Translation2d relativeTrl = aimTarget
                    .get()
                    .relativeTo(swerveDrive.getPose())
                    .getTranslation();
                Rotation2d target = new Rotation2d(relativeTrl.getX(), relativeTrl.getY()).plus(
                    currentHeading
                );
                omegaRadiansPerSecond = swerveController.headingCalculate(
