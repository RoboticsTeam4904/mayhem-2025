...
public class SwerveDriveTest {
     ...
     */
    private static final MutLinearVelocity m_velocity = new MutLinearVelocity(
        0,
        9,
        MetersPerSecond
    );
     ...
     */
    private static final MutAngularVelocity m_angVelocity = new MutAngularVelocity(
        0,
        0,
        DegreesPerSecond
    );
    ...
    public static double findDriveMotorKV(
        ...
        for (double kV_new = 0; kV_new < maxVolts; kV_new += 0.0001) {
            ...
            while ((Timer.getFPGATimestamp() - startTimeSeconds) < testDelaySeconds && !foundkV) {
                for (int i = 0; i < modules.length; i++) {
                    if (
                        (modules[i].getDriveMotor().getPosition() - startingEncoders[i]) >
                        minMovement
                    ) {
                        foundkV = true;
                        break;
     ...
     */
    public static double findCouplingRatio(
        SwerveDrive swerveDrive,
        double volts,
        boolean automatic
    ) {
        ...
        for (SwerveModule module : swerveDrive.getModules()) {
            ...
            if (automatic) {
                ...
                while (
                    !Rotation2d.fromDegrees(absoluteEncoder.getAbsolutePosition()).equals(
                        startingAbsoluteEncoderPosition
                    )
                );
                module.getAngleMotor().setVoltage(0);
            } else {
            ...
            DriverStation.reportWarning(
                module.configuration.name + " Coupling Ratio: " + couplingRatio,
                false
            );
    ...
    public static void logDriveMotorActivity(
        ...
        SmartDashboard.putNumber(
            "swerve/modules/" + module.configuration.name + "/SysId Drive Power",
            power
        );
        SmartDashboard.putNumber(
            "swerve/modules/" + module.configuration.name + "/SysId Drive Position",
            distance
        );
        SmartDashboard.putNumber(
            "swerve/modules/" + module.configuration.name + "/SysId Drive Velocity",
            velocity
        );
        log
            .motor("drive-" + module.configuration.name)
            ...
            new SysIdRoutine.Mechanism(
                (Voltage voltage) -> {
                    ...
                    SwerveDriveTest.powerDriveMotorsVoltage(
                        swerveDrive,
                        Math.min(voltage.in(Volts), maxVolts)
                    );
                },
                log -> {
                    for (SwerveModule module : swerveDrive.getModules()) {
    ...
    public static void logAngularMotorActivity(
        ...
        SmartDashboard.putNumber(
            "swerve/modules/" + module.configuration.name + "/SysId Angle Power",
            power
        );
        SmartDashboard.putNumber(
            "swerve/modules/" + module.configuration.name + "/SysId Angle Position",
            angle
        );
        SmartDashboard.putNumber(
        ...
        log
            .motor("angle-" + module.configuration.name)
            .voltage(m_appliedVoltage.mut_replace(power, Volts))
